...
public class Option {
     ...
     * html   * redirect link   * link for a dynamic offer   * raw json   * one or more actions (json - from offers
     * with templates and visual offers) Actions format is specific for delivery API.
     *
     * @return content
     **/
    ...
    /**
     * Will be present only in response of a prefetch request. After the content is displayed the event token should
     * be sent via notifications to the edge server so that visit/visitor/impression events could be logged.
     *
     * @return eventToken
     **/
    ...
    /**
     * List of the response tokens and their values for the given option. Response tokens can be defined via the
     * /v1/responsetokens API. The values for the tokens are computed for every option returned by a activity and
     * represented as a dictionary:   * Key - the response token name.   * Value - the response token value. The value
     * is usually a string, but it can be a list of string in case of   &#39;category affinity&#39; response token.
     *
