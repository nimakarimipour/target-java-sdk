...
public class PageLoadResponse {
    ...
    /**
     * List of options that should be applied. Note: options that have been set on selectors assigned to views will
    ...
    /**
     * Click track metrics. Note that click metrics for selectors set with views will not be included here. In case
     * the same activity is serving content for selectors both assinged to a view and selectors without any views, and
     * having click track metrics that are not assotiated with any view within the activity, then:   * in case of a
     * prefetch only request, these metrics (tokens) will be set in the Prefetch response&#39;s metrics.   * in case
     * of an execute only request, the metrics will be set in the page load response&#39;s metrics.   * in case of a
     * request, with both, execute and prefetch, metrics will be set in the page load response&#39;s     metrics only.
     *
     * @return metrics
     **/
    ...
    /**
     * View state token that must be sent back with display notification for the view. May only be present for
     * prefetch requests.
     *
     * @return state
     **/
